/*
	...
	지역별 확진자 수를 입력받아 어제 확진자 수, 오늘 확진자 수, 변동 인원(오늘 확진자 수 - 어제 확진자 수), 누적 인원(누계)을 지역별로 출력해줍니다.
	(새로 입력받은 확진자 수 = 오늘, 이전에 입력받은 확진자 수 = 어제)
	
	지역별 오늘 확진자 수 중 가장 큰 값을 괄호로 표시하여 출력해주었습니다.
	지역별 누적 확진자 수 중 가장 큰 값을 괄호로 표시하여 츨력해주었습니다.
	
	class를 통해 확진자 수를 계산하고 출력해주었습니다.
	확진자 수를 입력받을 때 음수일 경우 try-catch문을 통해 예외처리를 해주었습니다.
*/
...
void Covid19Counter::update(int* input) // 지역별 확진자 수를 최신화한다.
{
	...
	// 지역별 오늘 확진자 수 중 가장 큰 값을 찾는다.
	// 지역별 누적 확진자 수 중 가장 큰 값을 찾는다.
	for(int i = 0; i < 17; i++)
	{
		if(today[i] > todayMax)
		{
...
void Covid19Counter::print() // 지역별로 어제 확진자 수, 오늘 확진자 수, 변동인원(오늘-어제), 누적인원(누계)를 출력한다.
{
	...
	for(int i = 0; i < 17; i++)
	{
		if(today[i] == todayMax)   // 지역별 오늘 확진자 수 중 가장 큰 값은 괄호로 표시해준다.
		{
			cout << "\t" << "(" << today[i] << ")";
		}
		else
	...
	for(int i = 0; i < 17; i++)
	{
		if(total[i] == totalMax) // 지역별 누적 확진자 수 중 가장 큰 값은 괄호로 표시해준다.
		{
			cout << "\t" <<  "(" << total[i] << ")";
		}
		else
